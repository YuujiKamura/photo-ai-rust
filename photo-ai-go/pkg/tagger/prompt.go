...
const groupPromptTemplate = `工事写真を分類・グループ分けせよ。同一対象の写真をグループにまとめろ。Output ONLY JSON array: [{"file":"filename","role":"?","machine_type":"?","machine_id":"?","has_board":false,"detected_text":"","description":""}, ...]
ファイル: {list}
用語定義(重要):
...
// GroupPrompt builds the AI prompt for photo classification.
func GroupPrompt(filenames []string, vocabulary []string) string {
	list := strings.Join(filenames, ", ")
	prompt := strings.Replace(groupPromptTemplate, "{list}", list, 1)
	if len(vocabulary) > 0 {

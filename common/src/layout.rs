//! レイアウト設定モジュール
//!
//! GAS版 layoutConfig.ts と完全同一の設計思想
//! PDF生成時のレイアウト計算はGAS版 pdfCore.ts に準拠

// ============================================
// GAS準拠レイアウト定数（Source of Truth）
// ============================================

/// A4サイズ（pt）- GAS準拠
pub const A4_WIDTH_PT: f32 = 595.35;
...
/// マージン（pt）- GAS準拠: 28.35pt (10mm)
pub const MARGIN_PT: f32 = 28.35;

/// 写真間ギャップ（pt）- GAS準拠: 5pt
pub const GAP_PT: f32 = 5.0;

/// 写真/情報比率 - GAS準拠
pub const IMAGE_RATIO: f32 = 0.65;
pub const INFO_RATIO: f32 = 0.35;

/// ヘッダー高さ（pt）- GAS準拠: 0（ヘッダーなし）
...
/// ページサイズ（pt）
pub const PAGE_WIDTH_PT: f32 = A4_WIDTH_PT;   // 595.35pt
pub const PAGE_HEIGHT_PT: f32 = A4_HEIGHT_PT; // 842.0pt
...
/// ブロック高さ（pt）
pub const BLOCK_HEIGHT_3UP_PT: f32 = PHOTO_HEIGHT_PT_3UP + GAP_PT * MM_TO_PT; // 271.25pt
...
mod tests {
    ...
    fn test_gas_constants() {
        // GAS準拠定数の確認
        assert!((A4_WIDTH_PT - 595.35).abs() < 0.01);
        assert!((A4_HEIGHT_PT - 842.0).abs() < 0.01);
        assert!((MARGIN_PT - 28.35).abs() < 0.01);
        assert!((GAP_PT - 5.0).abs() < 0.01);
        assert!((IMAGE_RATIO - 0.65).abs() < 0.001);
        assert!((INFO_RATIO - 0.35).abs() < 0.001);
        assert!((HEADER_HEIGHT_PT - 0.0).abs() < 0.001);
    }

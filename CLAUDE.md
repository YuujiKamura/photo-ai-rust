...
- AIエージェントが作成する**あらゆるアウトプティブ（GitHub Issue、追記、PR、Zenn等への投稿ドラフト、外部メッセージ等）**において、必ず**自身のモデル名（例: Claude 3.5 Sonnet）**を自ら名乗ること。
- 「YuujiKamura名義の背後にいる実行主体」としてのアイデンティティを隠さず、透明性を確保すること。これは技術的なルールである以上に、エージェントが社会的な責任を果たすための基本動作である。
